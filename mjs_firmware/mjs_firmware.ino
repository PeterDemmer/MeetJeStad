/*******************************************************************************
   ...
   Added:
   - WITH_HPMA: HPMA115S0-master
   - WITH MJS: stuurt metingen naar MeetJeStad
 *******************************************************************************/
...
// #define WITH_LUX
// Enable this define when a Honeywell HPMA115S0 particle sensor is attached
#define WITH_HPMA
...
// setup APDS9310 light sensor
uint8_t const LUX2_ADDR = 0x39;
// setup HPMA particle meter
...
void setup() {
  ...
  // start communication to temp+hum sensor
  htu.begin();
  ...
  delay(100);
  hpmaStatus = hpma.stop_autosend();
  switch (hpmaStatus) {
      case 0: Serial.println("HPMA stop autosend ERROR"); break;
      case 1: Serial.println("HPMA stop autosend OK"); break;
      case 2: Serial.println("HPMA stop autosend timeout"); break;
  }
  delay(500);
  hpmaStatus = hpma.start_measurement();
  switch (hpmaStatus) {
      case 0: Serial.println("HPMA start measurement ERROR"); break;
      case 1: Serial.println("HPMA start measurement OK"); break;
      case 2: Serial.println("HPMA start measurement timeout"); break;
  }
#endif
...
#ifdef WITH_HPMA
    hpmaStatus = hpma.read(&hpma25, &hpma10);
    switch (hpmaStatus) {
      case 0: Serial.println("HPMA read ERROR"); break;
      case 1: Serial.println("HPMA read OK");
              Serial.print(F("HPMA PM2.5-value="));
              Serial.print(hpma25);
              Serial.print(F(", PM10-value="));
              Serial.print(hpma10);
              Serial.println(".");
              break;
      case 2: Serial.println("HPMA read timeout"); break;
    }
#endif
...
void loop() {
  ...
  Serial.print(F("voor dumpData: hpmaStatus="));
  Serial.println(hpmaStatus);
  if (DEBUG)
    dumpData();
  Serial.print(F("na dumpData: hpmaStatus="));
  Serial.println(hpmaStatus);
  ...
  if (DEBUG) {
    Serial.println();
    Serial.print(F("Woke up, Arduino time = "));
    uint32_t mymillis = millis();
    Serial.println(mymillis/1000.);
  }
}
...
void dumpData() {
  if (gps_data.valid.location && gps_data.valid.status && gps_data.status >= gps_fix::STATUS_STD) {
    Serial.print(F("Last GPS lat/lon: "));
    Serial.print(gps_data.latitudeL()/10000000.0, 6);
    Serial.print(F(","));
    Serial.print(gps_data.longitudeL()/10000000.0, 6);
    Serial.print(F(", date: "));
    Serial.print(gps_data.dateTime.year);
    Serial.print(F("-"));
    Serial.print(gps_data.dateTime.month);
    Serial.print(F("-"));
    Serial.print(gps_data.dateTime.date);
    Serial.print(F(", time: "));
    Serial.print(gps_data.dateTime.hours);
    Serial.print(F(":"));
